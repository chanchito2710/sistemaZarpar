...
const GlobalSales: React.FC = () => {
  ...
  // Estados para movimientos de caja (ajustes manuales)
  ...
  /**
   * Cargar ventas y movimientos de caja al montar
   */
  useEffect(() => {
    cargarVentas();
  ...
  const cargarSucursales = async () => {
    ...
    try {
      ...
      // Si es usuario normal, solo mostrar su sucursal
      if (esAdmin) {
        setSucursales(sucursalesData);
      } else {
        // Filtrar para mostrar solo la sucursal del usuario
        const sucursalDelUsuario = sucursalesData.find(
          (s: string) => s.toLowerCase() === sucursalUsuario
        );
        setSucursales(sucursalDelUsuario ? [sucursalDelUsuario] : [sucursalUsuario]);
      }
    } catch (error) {
  ...
  /**
   * Cargar movimientos de caja (ajustes manuales)
   */
  const cargarMovimientosCaja = async () => {
    ...
    try {
      ...
      if (fechaDesde) {
        params.fecha_desde = fechaDesde.format('YYYY-MM-DD');
      }
      if (fechaHasta) {
        params.fecha_hasta = fechaHasta.format('YYYY-MM-DD');
      }
      
      console.log('🔍 [MOVIMIENTOS CAJA] Parámetros de búsqueda:', {
        params,
      ...
      const data = await response.json();
      console.log('🔍 [MOVIMIENTOS CAJA] Respuesta del servidor:', data);
      
      if (data.success) {
        let movimientos = data.data || [];
        console.log('🔍 [MOVIMIENTOS CAJA] Total movimientos recibidos:', movimientos.length);
        console.log('🔍 [MOVIMIENTOS CAJA] Movimientos:', movimientos);
        
        // Filtrar por sucursal si está seleccionada y no es "todas"
        if (sucursalSeleccionada && sucursalSeleccionada !== 'todas') {
          console.log('🔍 [MOVIMIENTOS CAJA] Filtrando por sucursal:', sucursalSeleccionada);
          const movimientosFiltrados = movimientos.filter(
            (m: any) => m.sucursal.toLowerCase() === sucursalSeleccionada.toLowerCase()
          );
          console.log('🔍 [MOVIMIENTOS CAJA] Movimientos después de filtrar:', movimientosFiltrados.length);
          movimientos = movimientosFiltrados;
        }
        
        console.log('✅ [MOVIMIENTOS CAJA] Movimientos finales a mostrar:', movimientos.length);
        setMovimientosCaja(movimientos);
      } else {
        console.error('❌ [MOVIMIENTOS CAJA] Error en respuesta:', data);
      }
    } catch (error) {
      console.error('❌ [MOVIMIENTOS CAJA] Error al cargar:', error);
      message.error('Error al cargar movimientos de caja');
    } finally {
    ...
    <div className="global-sales-container">
      ...
      {!esAdmin && (
        <Alert
          ...
          description={`Solo puedes ver ventas de tu sucursal asignada: ${sucursalUsuario.toUpperCase()}`}
          type="info"
          showIcon
      ...
      {/* Tabla Unificada: Ventas y Movimientos de Caja */}
      <Card
        title={
          <span>
            <LineChartOutlined style={{ marginRight: 8 }} />
            Historial Financiero Completo ({ventas.length + movimientosCaja.length} registros)
          </span>
        }
        className="table-card"
      >
        {!loading && !loadingMovimientosCaja && ventas.length === 0 && movimientosCaja.length === 0 ? (
          <Empty description="No hay registros financieros" />
        ) : (
          <Table
            dataSource={[
              // Mapear ventas
              ...ventas.map((venta) => ({
                ...
                descripcion: venta.cliente_nombre,
                monto: venta.total,
                detalles: venta.metodo_pago,
              })),
              // Mapear movimientos de caja
              ...movimientosCaja.map((movimiento) => ({
                ...movimiento,
                tipo_registro: 'ajuste_caja',
                fecha: movimiento.created_at,
                referencia: `Ajuste #${movimiento.id}`,
            ...
            columns={[
              {
                ...
                render: (tipo: string) => (
                  <Tag 
                    color={tipo === 'venta' ? 'blue' : 'orange'}
                    style={{ fontSize: 10 }}
                  >
                    {tipo === 'venta' ? '💰 VENTA' : '⚙️ AJUSTE CAJA'}
                  </Tag>
                ),
                responsive: ['xs', 'sm', 'md', 'lg', 'xl'] as any,
              },
              ...
              },
              {
                ...
                render: (monto: number, record: any) => (
                  <Text 
                    strong 
                    style={{ 
                      color: record.tipo_registro === 'venta' ? '#52c41a' : '#1890ff', 
                      fontSize: 13 
                    }}
                  >
              ...
              },
              {
                ...
                key: 'detalles',
                width: 130,
                render: (detalles: string, record: any) => {
                  if (record.tipo_registro === 'venta') {
                    ...
                    return (
                      <Tag color={colores[detalles] || 'default'} style={{ fontSize: 10 }}>
                        {detalles.replace('_', ' ').toUpperCase()}
                      </Tag>
                    );
                  } else {
                    return (
                      <Text 
                        strong 
                        style={{ 
                          color: detalles.startsWith('+') ? '#52c41a' : '#f5222d',
                          fontSize: 11 
                        }}
                      >
                        {detalles}
              ...
              },
              {
                ...
                render: (_: any, record: any) => (
                  record.tipo_registro === 'venta' ? (
                    ...
                    <Tooltip title="Ajuste manual de caja">
